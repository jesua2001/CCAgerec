...
@Component({
  ...
  imports: [
    ...
    IonItem,
    IonLabel,
    IonSelect,          // ✅ necesario para <ion-select>
    IonSelectOption,    // ✅ necesario para <ion-select-option>
    NgForOf,
    NgIf,
    FormsModule,
    CommonModule
...
export class HomevgpartComponent implements OnInit {
  ...
  marca: string = '';
  numeroSerie: string = '';
  tipoMaquina: string = ''; // ✅ añadida para el select
  mensajeError: string = '';
  ...
  consultarHomevgparts() {
    ...
    if (!this.marca.trim()) {
      this.mostrarToast("La marca es obligatoria.");
      return;
    }
    if (!this.tipoMaquina) {
    ...
    this.maquina.obtenerEquivalencia(this.modelo, this.marca, this.numeroSerie, this.tipoMaquina).subscribe({
      next: (response: any) => {
